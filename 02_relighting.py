...
def main():
    ...
    p = 16
    q = 9
    dir_name = f"lighttransport_{n}x{m}x{cc}_{q}x{p}"
    print("Load light_transport_matrix")
    T = np.load(f"{dir_name}/light_transport_matrix.npy") # (m*n*cc, p*q)
    ...
    T_reshaped = np.reshape(T, (m*n, cc, p*q)) # (m*n, cc, p*q)
    imlist_output = []
    for i in range(cc):
    ...
    img_output = cv2.merge(imlist_output) # (n, m, cc)
